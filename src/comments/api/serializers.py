...
HyperlinkedIdentityField, SerializerMethodField,HyperlinkedRelatedField,ValidationError)
	...
	class CommentCreateSerializer(ModelSerializer):
		...
		def create(self,validated_data):
			...
			if user:
				...
				main_user = User.objects.all().first()#admin(superuser) id=1
			model_type = self.model_type
			slug = self.slug
...
"""
###########other_solution##############
#instead of create_comment_serializer function
#used with some commented code in comments/api/views.py  
class CommentCreateSerializer(ModelSerializer):
		class Meta:
			model = Comment
			fields = ['id','content','timestamp']	
...
class CommentSerializer(ModelSerializer):
	...
	class Meta:
		...
		fields = ['id','content_type','object_id',
			'parent','content','reply_count','timestamp']
...
class CommentDetailSerializer(ModelSerializer):
	...
	class Meta:
		...
		'replies','timestamp','content_object_url']
